...
| F-03 | provider.py tested end-to-end | Create provider from config, call `chat()` with tool schemas, verify `tool_calls` in response. Test with at least 1 provider (Anthropic or OpenAI) | CRITICAL |
...
| F-08 | Provider adapter for tool-calling format | Adapter translates OpenAI function-calling schema ↔ Anthropic `tool_use` / Gemini format so any provider can call tools | HIGH |
