...
export default function SystemStatusCard({ name, metric, value, max, status, detail, tooltip, awaiting }: SystemStatusCardProps & { awaiting?: boolean }) {
  const pct = max > 0 ? (value / max) * 100 : 0;
  if (awaiting) {
    return (
      <div className="rounded-xl border bg-card px-3 py-2 space-y-1 opacity-60 border-dashed">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-medium">{name}</span>
            <span className="text-[10px] text-muted-foreground">· awaiting ingest</span>
        ...
        <p className="text-[10px] text-muted-foreground italic">Will populate after webhook event</p>
